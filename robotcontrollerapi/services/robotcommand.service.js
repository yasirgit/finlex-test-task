...
const CurrentPositionModel = require('../models/currentposition.model');

// Save a new command for robot
exports.create = (newCommand, result) => {
    ...
    sql.query("INSERT INTO commands SET ?", newCommand, (err, res) => {
        ...
        sql.query("SELECT * FROM current_position ORDER BY id DESC LIMIT 0,1", (err, res) => {
            ...
            if (res[0]) {
                ...
                current_position = new CurrentPositionModel({
                    position_x: pos_x,
                    position_y: pos_y,
...
exports.currentPosition = result => {
    sql.query("SELECT * FROM current_position ORDER BY id DESC LIMIT 0,2", (err, res) => {
        ...
        const currentPositionData = {
            ...
            'current-position-commands': res[0].position_turn_x + ',' + res[0].position_turn_y,
            'previous-position': res[1].position_x + ',' + res[1].position_y,
            'previous-position-commands': res[1].position_turn_x + ',' + res[1].position_turn_y,
